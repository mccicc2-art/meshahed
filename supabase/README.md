...
| 56 | `account_deletion.sql` | حذف الحساب يحذف الحساب فعلاً: `delete_my_account` صارت **صفّاً واحداً** يحذف من `auth.users` فيجرّ الأربعة والأربعين مفتاحاً cascade — بدل قائمةٍ من أحد عشر جدولاً بقيت وراءها الرسائل والمجتمعات والحظر والأغلفة (D-146). **⚠️ نسخة `security.sql` §٦ حُدّثت لتطابقها** | **معلّقة** |

كلها تحقَّقت بالاستعلام المكتوب في ذيل ملفّها — **كل ملفات الجدول شُغِّلت
حتى ٥٥** (٥٢–٥٥ في ١٠ أغسطس). **٥٦ معلّقة** حتى تُشغَّل. تحقُّق ٥٥ على الإنتاج:
