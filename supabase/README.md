...
| 72 | `public_reads.sql` | **غرفةُ الكلام تُقرأ بلا حساب** (D-221): منحُ `execute` لـ`anon` على أربع دوالِّ قراءةٍ فقط — `title_reviews` · `title_replies` · `title_review_likes` · `community_rating`. **والحراسةُ داخلها ولا تحتاج سطراً**: `can_view_profile` بلا هويّةٍ يمرّ منها **الحسابُ غيرُ الخاصّ وحده**، و`hide_name` والمبلَّغُ عنه كما هما. **ولا كتابةَ ولا إعجابَ ولا بلاغَ لغير المسجَّل.** ⚠️ **ويوسّع «الآخرين» في وعد الخصوصية من المسجَّلين إلى الإنترنت — فنصُّ `/privacy` يجب أن يُحدَّث** | **معلّقة — لوحةُ Supabase لم تُفتح في الجلسة** |
