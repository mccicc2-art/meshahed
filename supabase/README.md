...
| 60 | `imdb_pool_flags.sql` | علَما البِركة: `is_doc` (يُلغي ~٨٠ نداء TMDB لكل رفّ «أفضل ٥٠»، D-165) + **رفعُ شرط الأفلام عن `is_anime` في `build_imdb_chart`** فيصير لأفلام الأنمي صنفُها (D-169/D-170). ⚠️ **آمنةٌ بلا أثر حتى تُشحن الشيفرة وتُعاد تعبئة البِركة** (لا صفَّ يحمل العلَمين بعد) | **معلّقة** |
