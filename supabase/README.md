...
| 28 | `person_follows.sql` | متابعة الفنانين — جدولٌ مستقلّ عن `follows` لأن معرّفات الأشخاص فضاءٌ آخر | زرّ «تابِع» في صفحة الشخص + صفّ «جديد من تتابعهم» في اكتشف |
| 29 | `profile_visibility.sql` | «من يرى آرائي» — عام أو المتابِعون، ويُحترَم في خطّ المجتمع وفي صفحة العمل | خيار الخصوصية في الإعدادات |
| 30 | `security3.sql` | **أمن:** إغلاق قراءة `follows` و`watched_episodes` و`watched_movies` — كانت مفتوحة لكل مسجَّل دخول | — |
