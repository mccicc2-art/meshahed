 ...
 *
 * 🔴 **وحدُّ ما تحقّق اليوم يُقال بلا تجميل — الهدفُ نصفُه مبنيّ:**
 * **الصفُّ والصفحةُ يتشاركان الدالّة فعلاً في `most-popular` و`airing-now`
 * وحدهما.** أمّا `top-ten` و`upcoming` و`in-cinemas` و`from-artists` فصفحتُها
 * تبني من هنا **وصفُّها ما زال يبني بنفسه في `news/page.tsx`** — فهي
 * **مصدران متطابقان اليوم وقابلان للانحراف غداً**، وهو بالضبط ما وُجد هذا
 * الملف لمنعه. **وتحويلُها بندٌ مُعلَن في `05_Todo` لا شيفرةٌ مؤجَّلة**:
 * أُبقيت لأن مسارَي «القادم» و«السينما» متشابكان برسمِ العدّ التنازلي
 * وبالمنطقة، وتحويلُهما في نفس الدفعة كان سيخلط تغييرين في ملفٍّ واحد.
 * **ومن يقرأ هذا الملف اليوم لا يظنّ أن العمل تمّ.**
 */
