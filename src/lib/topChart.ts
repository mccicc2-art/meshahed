 ...
 */
const MIN_CHART_VOTES = { movie: 25_000, tv: 20_000, anime: 10_000 } as const;

/**
 * ⚖️ 🆕 **وعتبةُ الأنمي وحدَها نزلت إلى عشرة آلاف** (D-385، موافقةُ أحمد
 * على البابِ الثاني من D-382).
 *
 * **نقضٌ صريحٌ لتطبيق D-323/D-365 على الأنمي — ويُسجَّل باسمه**، **وحُجّةُ
 * القاعدة الأمِّ باقية**: «رقمٌ يكذب أسوأُ من لا رقم» (D-219). **والذي
 * تغيّر أن العشرين ألفاً كانت تحرس معنًى واحداً في بِركتين مختلفتين**:
 * فيلمٌ إنجليزيٌّ يبلغها في شهر، **وأشهرُ الأنمي كلِّه ٢٠٦ أعمالٍ فوق
 * الخمسة آلاف في IMDb كلِّها** (D-382) — **فالعتبةُ تُقاس بالبِركة التي
 * تقف عليها لا برقمٍ يُنسخ بين الأصناف** (D-378 حرفاً).
 *
 * **والمقيس قبل القرار**: `≥20k = 136` · `≥15k = 165` · **`≥10k = 188`** ·
 * `≥5k = 206`. **فالعشرةُ آلاف تكسب اثنين وخمسين عملاً** ولا تنزل إلى
 * القاع.
 *
 * ⚠️ **والفيلمُ والمسلسلُ لم يُمَسّا** — بل صار الرقمُ هنا يطابق جسمَ
 * `build_imdb_chart` حرفاً (٢٥٬٠٠٠ للفيلم · ٢٠٬٠٠٠ للمسلسل)، **وكان
 * القارئُ يحرس بعشرين ألفاً والبناءُ بخمسةٍ وعشرين** — **رقمان لمعنًى
 * واحدٍ في موضعين** (D-216).
 */
