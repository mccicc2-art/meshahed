...
/* ⚖️ 🆕 D-1118 — **المحاولةُ الأولى ٢٥٠٠ لا ٥٠٠٠** (أحمد بتسجيل: صفحةُ عملٍ هيكلٌ ستَّ ثوانٍ ومواسمُ لا تظهر).
   ردُّ TMDB السليم دون نصف ثانية؛ **اتصالٌ عالقٌ هو ما يأكل المهلةَ كلَّها** ثمّ تنجح الثانية — فالمهلةُ
   الأولى ثمنُ كلِّ تعليقةٍ لا ثمنُ الردِّ البطيء. الثانيةُ باقيةٌ ٨٠٠٠ فحجّةُ D-580 («بياناتُها هي الصفحة،
   والانتظارُ أصدقُ من صفحةٍ فارغة») قائمة: أسوأُ حالةٍ ~١١ث لا ~١٣٫٥ث، وأشيعُها ~٣ث لا ~٦ث. */
...
export async function topTenThisWeek(
  ...
  const rows = (data.results ?? [])
    ...
    /* **ولا تصفيةَ أنميٍ هنا بعد اليوم (D-175).** كانت هنا: تُسقط من
       المسلسلات كلَّ ما نوعُه «رسوم» (١٦). وسقطت لسببين:

       ١) **كانت تعمل في مسارٍ واحدٍ من ثلاثة.** هذه الدالة مسارُ «هذا
          الأسبوع بلا فلتر» وحده — وبمجرّد اختيار نوعٍ أو نافذةِ شهرٍ أو
          سنة تُنادى `topTenGenreThisWeek` أو `topByFilter` بدلاً منها،
          **فيعود الأنمي**. («المحقّق كونان» رابعَ «أفضل ١٠ مسلسلات» في
          «جريمة · التسعينات» على الموقع الحيّ، ١١ أغسطس.)
       ٢) **وكانت تُسقط ما لا يُراد إسقاطه.** النوع ١٦ وحدَه يشمل
          «ريك آند مورتي»، **وأحمد طلب بقاءه صراحةً**: «إزالة الأنمي
          **الياباني**… فيه قائمة للإنمي الياباني». والقاعدةُ الصحيحة
          (نوع ١٦ **ولغةٌ يابانية**) مكتوبةٌ في `looksAnime` وفي
          `topChart.filterRail` — فكانت هذه نسخةً ثالثةً تخالفهما.

       فالتصفيةُ صعدت إلى موضع النداء في `news/page.tsx` بـ`looksAnime`،
       **حيث تمرّ المساراتُ الثلاثة كلُّها**، وحيث يقف رفُّ الأفلام أصلاً. */
...
/**
 * **الأكثرُ شعبيةً ممّا يُعرف فعلاً** — شعبيةٌ بحاجزِ جودةٍ وحاجزِ أصوات
 * (D-202، بلاغ أحمد بلقطة: صفُّ «الأكثر شعبية» كان **Secret Story ٤٫٦ ·
 * Paradise Hotel ٤٫٣ · Paradise Hotel ٤٫١ · Big Brother**).
 *
 * **والسببُ في المقياس لا في التطبيق:** `popularity` عند TMDB **رقمُ حركةٍ
 * لا رقمُ جودة** — يُحسب من المشاهدات والبحث والإضافات في اليوم. وبرامجُ
 * الواقع الأوروبية تُنتج نسخةً وطنيةً كلَّ موسم، **فتتصدّر الحركةَ يومياً
 * بأصواتٍ قليلةٍ وتقييماتٍ متدنّية**. فقائمةُ `/popular` العارية تُعطي
 * «الأكثرَ حركةً»، **وأحمد يطلب «الأكثرَ شعبيةً ومعروفاً جداً» — وهما
 * ليسا الشيءَ نفسه.**
 *
 * **فحاجزان، وكلُّ واحدٍ يقتل نصفَ المشكلة:**
 * - `vote_count.gte` — **«معروفٌ جداً» تعني أن كثيرين قيّموه**، لا أن
 *   كثيرين فتحوه أمس. وهو ما يُسقط النسخَ الوطنية: لكلٍّ صفٌّ منفصل عند
 *   TMDB بأصواتٍ معدودة (**ولهذا ظهر «Paradise Hotel» مرّتين**).
 * - `vote_average.gte` — حاجزُ جودةٍ صريح. **٦٫٥ لا ٧٫٥:** الأعلى يُسقط
 *   أعمالاً جماهيريةً حقيقية، والصفُّ ليس «أفضل ٢٥٠» — له رفُّه.
 *
 * **والصفحتان لا واحدة:** الحاجزان يُسقطان كثيراً، فبِركةٌ بحجم المطلوب
 * تعود ناقصة — **وهو درسُ D-185 حرفياً** (بِركةٌ بحجم الحدّ تُفسد الحاجز
 * الذي فوقها).
 */
export async function popularWellKnown(
  mediaType: MediaType,
  f: DiscoverFilter,
  pages = 2,
  minVotes = 1500,
  ...
  const rungs: { votes: number; rate: number }[] = [
    ...
    { votes: 300, rate: 6 },
    { votes: 50, rate: 0 },
    /* 🔴 **ودرجةٌ بلا حاجزِ أصوات — للبِرَك الضيّقة وحدَها** (D-378،
       قِيس على الموقع: «الأكثر شعبية» بالعربية أعطت **أربعة** عند
       `vote_count >= 50`). **والحاجزُ وُضع ليمنع نكرةً من تصدّر رفٍّ
       عالميّ** (D-202)، **وفي بِركةٍ لغتُها مختارةٌ يمنع الجوابَ نفسَه**
       — **و«الأكثر شعبيةً بالعربية» يعني ترتيبَ العربيّ بالشعبية، لا
       العربيَّ الذي بلغ حاجزاً عالميّاً.**
       ⚠️ **ولا تُبلَغ إلا بعد أن تعجز الثلاثُ فوقها**، فالرفُّ العالميُّ
       لا يمرّ بها أصلاً. */
    { votes: 0, rate: 0 },
  ];
