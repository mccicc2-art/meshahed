...
/** نافذة صفوف «أفضل ١٠» — لكل صفٍّ نافذته (D-099، طلب أحمد): أزرار
    ...
    الفلاتر (يعدّل D-056/D-075). و«شهر» = آخر ثلاثين يوماً.

    🆕 ⚖️ **و«كل الأوقات» صارت نافذةً ثالثةً بدل «سنة» — نقضٌ مزدوجٌ
    يُقال كاملاً** (D-445، مواصفةُ المرحلة ٦: «Top 10 This Week ·
    Week / Month / All time»):

    **الأوّل نقضُ D-420** التي شطبت الرقائقَ كلَّها بطلبِ أحمد على
    اللقطة («احذف فلتر أسبوع وشهر وسنة») — **وقد رجع عن ذلك بنصِّ
    المواصفة**، فالمبدِّلُ يعود بثلاث نوافذ لا بأربع.
    **والثاني نقضُ «سنة» نفسِها**: كانت تعني السنةَ الميلاديّة الجارية،
    **وذاك سؤالٌ يجيبه ذيلُ «أفضل ٢٥ هذي السنة» أصلاً** — فنافذتان
    لسؤالٍ واحدٍ في صفحةٍ واحدة (قاعدة ٦). **و«كل الأوقات» سؤالٌ لا
    يجيبه شيءٌ في الصفحة اليوم.**

    ⚠️ **و«سنة» باقيةٌ في النوع لدفعةٍ واحدة لا أكثر** (D-028): فروعُها
    مكتوبةٌ في `app/` وتُرفع بعد هذا المجلَّد، **ونوعٌ يضيق قبل قارئه
    يكسر الدفعةَ الوسيطة.** تسقط في الدفعة التي تليها. */
export type RailWin = "week" | "month" | "all" | "year";

/** قراءة نافذة صفٍّ من معامل رابط (wm/ws/wa) — المجهول يسقط لأسبوع.
    **و«سنة» لم تعد تُقرأ**: روابطُها القديمة تهبط على الأسبوع بدل أن
    تعرض نافذةً لا يملك المبدِّلُ زرّاً يعود منها (حالةٌ ميتة). */
