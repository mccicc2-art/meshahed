...
/**
 * 🔴 🆕 **و«المفضّلة» صنفٌ في القاعدة منذ الهجرة ٥٥ ولم تكن في النوع**
 * (D-654): `user_lists.kind` يقبل `favorites` **وقيدُ الجدول يعرفها**،
 * **والنوعُ هنا كان يجهلها** — **فكلُّ فرعٍ يسأل عن الصنف كان أعمى
 * عنها**، و`getList` تصبّ `kind` قسراً فتقرأ قائمةَ المفضّلة «عاديّة».
...
/**
 * 🆕 **قوائمُك كما تُعرض — بلا «المفضّلة»** (D-654، طلبُ أحمد: «واللستة
 * المفضّلة احذفها» مع «حطّ أيقونة قلب… يظهر لي الأفلام المفضّلة»).
 *
 * 🔑 **ولا تُحذف من القاعدة ولا صفٌّ واحد**: **هي وعاءُ ما يعرضه القلبُ
 * نفسُه** — **فحذفُها حذفُ ما طلب أن يراه.** **والذي سقط ظهورُها بطاقةً
 * بين قوائمه**: صار لها بابُها، **وشيءٌ واحدٌ في مكانين يُقرأ شيئين**
 * (القاعدة ٣/D-374).
 *
 * ⚠️ **ودالّةٌ واحدةٌ لقارئَيها** (`/library` و`/lists`) — **وهما
 * الشاشةُ نفسُها بطريقين** (تعليقُ `LibraryGrid`: «نفس تركيبة صفحة
 * /lists حرفياً — لا نسخة ثانية منها»)، **ولو صُفّيت في واحدةٍ
 * لافترقتا عند أوّل تعديل** (D-145).
 *
 * ⚠️ **ولا تُصفَّى في `getMyLists` نفسِها**: منتقي «أضِف إلى قائمة» في
 * صفحة العمل يقرؤها — **وإخفاءُ «المفضّلة» هناك يمنعك من التفضيل
 * أصلاً.** **الحجبُ عرضٌ لا مصدر.**
 */
export function listsForDisplay(lists: UserList[]): UserList[] {
  return lists.filter((l) => l.kind !== "favorites");
}
