...
export interface TalkStat {
  replies: number;
  watchers: number;
  /** كم شخصاً وضعه في مفضّلته (هجرة ٧٠) — **`0` قبل تشغيلها** */
  favorites: number;
}

/**
 * عدّاداتُ صفّ «الأعمال»: كم ردّاً وكم مشاهداً وكم مفضِّلاً — **بنداءٍ
 * واحد للصفّ كلّه** (D-193). والمفتاح `"<media>-<id>"` كما في بقيّة
 * الخرائط.
 *
 ...
 *
 * **والمفضِّلون أُضيفوا في D-216** بطلبه. **وقراءتُهم متسامحةٌ عمداً:**
 * الدالّةُ قبل هجرة ٧٠ تُرجع أربعة أعمدة لا خمسة، **فالغائبُ صفرٌ لا
 * انهيار** — والصفرُ لا يُرسم أصلاً. **قاعدةُ العمود الجديد: شيفرةٌ قد
 * تصل قبل هجرتها فيجب أن تعمل بدونها.**
 *
 * ⚠️ **وما ليس هنا عمداً: عددُ المقيّمين.** النجمةُ على البطاقة متوسّطُ
 * **من تكلّم في هذا الخطّ** (D-193)، **فمقامُها يُحسب من نفس تلك الآراء
 * في `groupByWork`** لا من عدّ كلِّ من قيّم في التطبيق. **بسطٌ من قومٍ
 * ومقامٌ من قومٍ آخرين يعطي رقماً يبدو دقيقاً وهو غلط.**
 */
