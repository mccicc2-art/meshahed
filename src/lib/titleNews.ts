...
/**
 * ⚠️ **`tonight` و`soon` حُذفا** (D-214، بلاغُ أحمد): «موعدُ نزول الحلقة
 * القادمة **موجودٌ أصلاً في Upcoming**» — **وتكرارُه هنا ضجيجٌ بلفظٍ آخر**
 * (قاعدةُ D-212: الخبرُ ما لا يعرفه المستخدم من مكانٍ آخر).
 * **و`aired` بقيت عمداً:** «نزلت حلقةٌ جديدة» **ماضٍ لا موعد**، وليست في
 * Upcoming، **وهي أنفعُ سطرٍ في التبويب لمن يتابع مسلسلاً**.
 */
...
export async function getTitleNews(): Promise<NewsItem[]> {
  ...
  for (const f of follows) {
    ...
    let kind: NewsKind;
    if (f.media_type === "tv") {
      /* **الحلقةُ القادمة لا تُذكر هنا** (D-214): Upcoming يقولها.
         والماضي وحده خبر — «نزلت حلقة». */
      if (d >= 0) continue;
      kind = "aired";
    } else {
