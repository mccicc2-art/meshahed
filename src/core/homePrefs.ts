...
/**
 * 🆕 **مفتاحُ بطاقة «للمشاهدة» في ترتيب صفِّ القوائم** (D-866) —
 * **طابورٌ لا صفَّ له في `user_lists`** (D-559) **فمفتاحُه اسمُه**،
 * ولا يصطدم بمعرّف قائمةٍ (UUID). **ويُكتب هنا لا في الصفحة**:
 * الكاتبُ والقارئُ يقرآنه من موضعٍ واحد (D-145).
 */
export const TW_LIST_KEY = "tw-queue";
...
/**
 * 🆕 **ما بقي في «للمشاهدة» — والمشاهَدُ يسقط** (D-848، حكمُ أحمد:
 * «أيّ فلم تمّت مشاهدته يسقط من قائمة تو واتش»).
 *
 * 🔑 **والقائمةُ تصير اسمَها**: **طابورُ «للمشاهدة» كان يعني «أفلامُك
 * التي لا قائمةَ لها»** (D-559) — **وذلك تعريفُ مصدرٍ لا تعريفُ
 * قائمة** — **فكان يعرض فيلماً شاهدتَه تحت عنوانٍ يقول «للمشاهدة».**
 * **وقائمةٌ تخالف اسمَها عطلٌ لا تنويع** (D-217).
 *
 * ⚠️ **والمشاهَدُ يسقط من العرض لا من الحساب**: **بطاقةُ «تابِع
 * المشاهدة» تبقى تقول ٣١/٤٢ · ٧٤٪** (حكمُه حين عُرض عليه الخياران) —
 * **فالقائمةُ تقول ما بقي، والبطاقةُ تقول كم قطعتَ** — **ولذلك
 * تُنادى هذه على الطابور الكامل ولا تحلّ محلَّه.**
 *
 * ⚠️ ~~**وقارئان لا ثالث**~~ — **ثلاثة** (D-942): صفحةُ `/lists` كانت
 * القارئَ الثالثَ المنسيَّ، وظهر فيها المشاهَدُ حتى بلّغ أحمد.
 * (الرئيسيّة والمكتبة و`/lists`): **مداخلُها مختلفة**
 * (`movieFollows` هناك و`follows` هنا) **والذي وُحّد هو الحكمُ لا
 * الجلب** — **وحكمٌ يُكتب مرّتين يفترق مرّةً، فتقول صفحةٌ ما لا تقوله
 * أختُها عن القائمة نفسِها.**
 */
export function unwatchedOf<T extends { tmdb_id: number }>(
  queue: readonly T[],
  watchedIds: ReadonlySet<number>,
): T[] {
  return queue.filter((f) => !watchedIds.has(f.tmdb_id));
}
