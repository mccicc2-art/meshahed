...
export default async function PublicProfilePage({
  ...
  const tabItemsAll: PageTab[] = [
       ...
       منذ D-667**، **والصفحةُ تفتح على أوّلِ ظاهرٍ لا على اسمٍ بعينه.** */
    {
      ...
      icon: "heart",
      /* 🔴 **`?tab=favorites` لا `base`** (D-940b، بلاغُ أحمد: «أضغط على favorites
         لكن لا يعمل»): كان رابطُها الجذرَ العاري **لأنها كانت البابَ الأوّلَ دائماً**
         (D-658) — **ولمّا صار الأوّلُ ترتيباً لا اسماً** (D-940) صار الجذرُ يفتح
         «أوّلَ ظاهر» أيّاً كان، **فضغطةُ «المفضّلة» تعيدك إلى «نظرة عامة».**
         **الرابطُ يسمّي وجهتَه كإخوته.** */
  ...
  return (
    ...
    <div
      ...
      /* 🔴 🆕 D-1132 — **التبويباتُ تلتصق بحافّة الشاشة لا تحت ترويسةٍ غائبة** (أحمد بخطٍّ أزرق وسهم:
         «ارفع البروفايل إلى الخط الأزرق»): `--sticky-top` = ترويسةٌ (٤rem) + شريطُ الحالة، والترويسةُ
         مخفيّةٌ هنا منذ D-643 — فالتصق الشريطُ ٦٤ تحت الحافّة، و`StickyStuck` ملأ ما فوقه بلون الأرضيّة:
         شريطٌ أسودُ فارغ. **يُعاد تعريفُه على جذر الصفحة وحدَها** (المتغيّراتُ تُورَّث — حجّةُ D-825)،
         فيبقى صحيحاً في كلِّ صفحةٍ ترويستُها ظاهرة. */
      style={{ ...densityVars(prefs.density), "--sticky-top": "var(--safe-top)" } as React.CSSProperties}
    >
      {/* ===== الغلاف ===== */}
      <section>
        ...
        {/* ⚖️ 🆕 **ودرجةٌ ثالثةٌ للشاشة الواسعة** (D-836، بلاغُ أحمد
            ...
            ⚖️ 🆕 **ثمّ لزمت الدرجةُ الثانية** (D-867): **الحاويةُ صعدت
            من ١١٥٢ إلى ١٤٤٠** — **فالعمودُ ١٤٠٨ والارتفاعُ ٣٥٢ يعطيان
            ٤٫٠:١** — **وهو الشريطُ الرفيعُ الذي أسقطه D-836 بعينه.**
            **فالدرجةُ تتبع الحاويةَ لا تُترك خلفَها**: `xl` عند ٤٠٠px
            (٣٫١–٣٫٦:١ في مداها) و`2xl` عند ٤٣٢px (**١٤٤٠÷٤٣٢ = ٣٫٣:١**
            — نسبةُ D-836 نفسُها عند السقف الجديد). */}
        {/* ⚖️ D-1110 — **الغلافُ على الهاتف ٧٫٧٥rem لا ٩٫٥** (أحمد بخطٍّ أزرق على لقطته: «ارفع
            البروفايل إلى الخط الأزرق»): الهويّةُ تصعد ~٢٨px فتظهر البطاقةُ والتبويباتُ أعلى.
            **والدرجاتُ الأعرض لم تُمسّ** — الحجّةُ عن الشاشة الضيّقة وحدَها.
            🆕 D-1132 — **ثمّ ٦٫٢٥rem** (أحمد بخطٍّ أزرق تحت سهم الرجوع: «فيه مساحة فاضية»): الهويّةُ
            تصعد ٢٤px أخرى، فتبدأ الصورةُ الشخصيّةُ (`-mt-10`) على بُعد ~١٠px تحت هدف لمس الرجوع
            (`safe-top` + ٦ + ٤٤) — أدنى من هذا وتلامسه. */}
        <div className="relative h-[calc(6.25rem+var(--safe-top))] sm:h-[15rem] lg:h-[22rem] xl:h-[25rem] 2xl:h-[27rem] -mx-4 -mt-6 sm:mx-0 sm:mt-0 sm:rounded-3xl overflow-hidden">
        ...
        {/* 📍 **وصفُّ الهويّة تحت الصورة كاملَ العرض** (D-854، حكمُ
            أحمد بلقطةٍ مرجعيّة: «الحساب وتاريخ الانضمام تحت الصورة بنفس
            التنسيق») — **خرج من العمود الأيمن (`flex-1`) إلى شقيقٍ للصفّ
            كلِّه**: كان يُحاذي الاسمَ فينحسر عن يسار الصورة، **وصار يبدأ
            من حافّتها** كما في اللقطة — **المحتوى نفسُه، الموضعُ وحدَه
            تغيّر** (والفراغُ `mt-3` لأنه صار يلي الصفَّ لا النبذة). */}
        ...
        {/* ⚖️ D-1110 — **على لون الصفحة بلا فواصل** (أحمد بلقطةٍ محوَّطة: «الخلفية سوداء وشيل
            الخط بين الأشياء — نفس النظام في الأماكن الثانية»): **نظامُ D-1093 في رأس الرئيسيّة
            الأصليّة بعينه** — الإطارُ الخارجيُّ وحده يجمع الخانات، **وبطاقتا أرقامٍ بشكلين في
            التطبيق نفسِه تُقرآن رتبتين.** */}
        {canView && prefs.stats && (
          <div className="relative z-10 mt-3 rounded-2xl border border-border bg-[color:var(--background)] overflow-hidden">
