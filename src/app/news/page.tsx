...
export default async function NewsPage({
  ...
  /* ⚖️ 🆕 **والبوّابةُ سقطت — اكتشف مفتوحٌ للضيف** (D-627، مواصفةُ
     أحمد: «أي أحد يدخل لوبز حتى وهو ما هو مسجّل يقدر يشوف الكوميونيتي
     والديسكفري») — **والصفوفُ الشخصيّةُ ذاتيّةُ الحراسة أصلاً**: كلُّ
     جلبٍ يقرأ الهويّةَ يعود فارغاً للزائر عبر RLS (نصُّ حجّة موجة
     الترويسة نفسِها)، **والكتابةُ محروسةٌ بـ`requireUser` في مكانها.** */
...
async function ListsDiscovery({
  ...
  /* ⚖️ 🆕 **ونُقض شطرا D-326 و D-348 بحكم أحمد** (D-366) («أوّل قلت لك الي أنا
     حاط لها قلب أخفيها — **أعتذر رجّعها**»).

     **يومَ كُتبا كانا صحيحين وسببُهما مكتوب**: المحفوظُ صار عندك
     فاقتراحُه ليس اكتشافاً، **والقائمةُ التي ظهرت في صفٍّ شخصيٍّ تسقط من
     رفِّ عالَمِها** فلا تُقرأ الصفحةُ مرّتين.

     🔴 **والذي أبطلَهما ثمنُهما لا حجّتُهما**: رفُّ «TOP 250» في لقطته
     يحمل **بطاقةً واحدة** — «أفضل ٢٥٠ أنمي» — لأن أختيها صعدتا إلى
     «تناسبك»، **ورفٌّ اسمُه «أفضل ٢٥٠» فيه واحدةٌ من ثلاثٍ يُقرأ نقصاً
     في القاعدة لا ترتيباً** (D-181 معكوسةً: الفراغُ الجزئيُّ يكذب أكثرَ
     من الغياب الكامل). **والرفُّ الثابتُ عنوانُه وعدٌ بمحتواه كلِّه**، لا
     ببقاياه بعد أن تأخذ الصفوفُ الشخصيّةُ نصيبَها.

     **فالحفظُ صار حالةَ بطاقةٍ لا سببَ اختفاء**: ♥ ممتلئٌ على البطاقة
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 🔴 🆕 **وقوائمُ المجتمع صعدت إلى المرتبة الثالثة** (D-368، طلبُ
          أحمد: «هذا يكون ثالث عنوان بعد ليست فور يو وترينديج ليست»).
          **وهو تصحيحُ ترتيبِ D-349 لا نقضُه**: ذاك رتّب «من الأخصِّ إلى
          الأعمّ، ومن المتحرّك إلى المستقرّ» **فأنزل المجتمعَ خلف اثنين
          وأربعين رفّاً ثابتاً** — **والمستقرُّ يُزار بابُه، والمتحرّكُ
          يُرى أو يموت** (D-299 بروحها): خطُّ الأعضاء يتغيّر كلَّ يوم
          **وأرشيفُ العوالم لا يتغيّر أبداً**، فالثابتُ يحتمل الانتظارَ
          والحيُّ لا يحتمله. **والثلاثةُ الأُوَل كلُّها أشخاص**: أنت، ثم
          نحن، ثم هم — **ثم الأرشيف.** */}
        ...
        rows.map((f) => {
          /* 🔴 🆕 **والرفُّ الفارغ لا يُرسم رأسَه** (D-363، بلاغُ أحمد
             ...
             **فيبقى الرأسُ يَعِد بما لا يأتي.** **ويُقاس القسمُ بما
             ...
             🆕 **وبقي سببٌ واحدٌ للسقوط** (D-366): **لم تُولَّد بعد.**
             **والظهورُ فوقُ لم يعد يُسقط** — رفُّ العالَم يعرض عالَمَه
             كاملاً. */
          const sets = f.sets.filter((u) => curated.has(u.slug));
...
function Top25View({
  ...
  return (
    <RankedRail
      ...
      items={rows}
      lib={lib}
      /* 🔴 **ولا «الكل» له — وهذا امتناعٌ مقصودٌ لا سهو** (D-827):
         **بُني له قسمٌ من مصدره نفسِه ثمّ قِيس فرُفض**: **الصفحةُ تعود
         بأحدَ عشرَ صفّاً والرفُّ باثنين وعشرين، ثابتاً في ثلاث
         قراءات** — **وبابٌ يَعِد بـ«الكل» ويعطي نصفَه أسوأُ من غيابه**
         (D-217/D-198 معاً).
         🔑 **والعلّةُ لم تُعزل بعد**: **`rankByImdb` تُسقط ما لا تقييمَ
         له** (D-323: من لم يبلغ الحاجزَ يسقط ولو فرغ الرفّ)، **وإثراءُ
         التقييمات يختلف بين مسارَي الطلب لسببٍ لم أثبته** — **ولا
         يُشحن بابٌ على فرضيّة.**
         ⬜ **والقسمُ باقٍ مصدراً للرفّ** (`top-25` في `sections.ts`):
         **ذاك مكسبٌ قائمٌ بذاته** — مصدرٌ واحدٌ بدل مصدرٍ في صفحة —
         **والبابُ يُفتح يومَ يصدق.** */
    />
  );
}
