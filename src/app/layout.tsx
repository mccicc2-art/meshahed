  ...
  return (
    ...
    >
      <head>
        ...
        {/* 🆕 **مصافحةُ يوتيوب تبدأ قبل أن يُطلب الإطار** (D-743، بلاغُ
            أحمد: «الفيديو ما يشتغل بسرعة»).
            🔑 **أوّلُ مقطعٍ يدفع ثمنَ ثلاث رحلاتٍ لا يراها أحد**: DNS
            ثمّ TCP ثمّ TLS لنطاقٍ لم نلمسه في هذه الصفحة قطّ —
            **وعلى شبكة الجوّال هذه مئاتُ الأجزاء من الثانية قبل أن
            يُطلب أوّلُ بايتٍ من الفيديو.** **و`preconnect` تدفعها
            بالتوازي مع رسم الصفحة بدل أن تُدفع بعد الضغط.**
            ⚠️ **ونطاقان لا ثلاثة**: الإطارُ وخادمُ صورِه. **وخادمُ
            المقاطع (`googlevideo`) اسمُه يُولَد لكلِّ جلسةٍ ولا يُعرف
            مسبقاً** — **و`preconnect` إلى اسمٍ مخمَّنٍ تحجز اتّصالاً لا
            يُستعمل أبداً فتزاحم ما يُستعمل.** (كتبتُه ثمّ حذفتُه في
            الجولة نفسِها: **الحدسُ لا يُشحن.**)
            ⚠️ **ولا تُكتب في صفحةٍ بلا ترايلر**: هي في الجذر لأن الصفَّ
            في «اكتشف» وهي أكثرُ ما يُفتح — **وثمنُها اتّصالٌ خاملٌ
            يُغلق وحدَه.** */}
        ...
        {/* 🆕 **ولونُ التمييز بعده فيغلبه** (D-825) — **ستّةُ متغيّراتٍ
            لا ثمانيةَ عشر**: **الخلفيّةُ والنصُّ ليسا للاختيار** (من
            ملكهما ملك أن يصنع تطبيقاً لا يُقرأ).
