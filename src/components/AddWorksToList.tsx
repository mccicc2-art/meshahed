...
export function AddWorksToList({
  ...
  return (
    <button
      ...
      onClick={run}
      disabled={pending}
      className={`inline-flex items-center gap-1.5 shrink-0 rounded-full border border-border bg-surface px-3.5 h-9 text-12 font-semibold text-foreground/85 hover:text-accent hover:border-accent/50 active:scale-95 disabled:opacity-50 transition ${className}`}
    >
