...
const SECTION_TONE = {
  ...
  rising: "text-[#FB923C]",
  /**
   * 🆕 💬 **الفيروزيُّ للعمل الذي يدور حوله الكلام** (D-291).
   * **والخمسةُ فوقه شغلت البنفسجيَّ والذهبَ والأحمرَ والأزرقَ
   * والبرتقاليّ** — **والفيروزيُّ آخرُ درجةٍ تُميَّز عنها كلِّها بنظرة.**
   * ⚠️ **وليس أخضرَ النجاح** (`#90CEA1`): ذاك حالةُ «شاهدته» وله معنًى
   * محجوز، **ولونٌ واحدٌ لمعنيين هو ما تمنعه القاعدة ٣.**
   */
  talked: "text-[#2DD4BF]",
} as const;
...
/**
 * 🆕 **«العملُ الذي يدور حوله الكلام» — بطاقةٌ عريضةٌ واحدة** (D-291،
 * اقتراحٌ في `05_Todo` أقرّه أحمد).
 *
 * ================= لماذا بطاقةٌ واحدةٌ لا قسمٌ سادس =================
 *
 * **الشكوى كانت «أحتاج شيئاً بينهم يكسر طابعَ التكرار»** (D-290)،
 * **ورفُّ القوائم كسره بالاتّجاه** — أفقيٌّ وسط شبكاتٍ رأسيّة.
 * **وهذه تكسره بالحجم**: بطاقةٌ واحدةٌ بعرض اللوحة كلِّها **وسط أقسامٍ
 * كلُّها ثلاثةُ عناصرَ في صفّ.** **وكسرانِ مختلفانِ خيرٌ من كسرٍ يتكرّر**
 * — **ورفٌّ ثانٍ كان سيصير هو نفسُه الإيقاعَ الذي يُكسر.**
 *
 * **وهي أيضاً الترتيبُ الوحيدُ في اللوحة الذي ليس عن شخص**: أربعةٌ
 * ترتّب الناسَ وما يصنعون، **وهذا يرتّب عملاً** — **فاختلافُه في
 * الشكل صادقٌ لأن اختلافَه في النوع حقيقيّ** (D-290: الكسرُ يقع
 * بالإيقاع، **وهنا الإيقاعُ والمضمونُ اختلفا معاً**).
 *
 * ================= ⚠️ والعنوانُ لا يقول «هذا الأسبوع» =================
 *
 * **الاقتراحُ المكتوب كان «العملُ الذي يتحدّث عنه الناس هذا الأسبوع»،
 * وسقطت منه «هذا الأسبوع» عمداً:** `title_talk_rooms` تُرجع **عدَّ
 * المشاركات كلَّه** لا عدَّ الأسبوع، **وعمودُ أسبوعٍ يوجب `drop`
 * للدالّة** — خارج الإذن الدائم. **وعنوانٌ يقول «هذا الأسبوع» فوق رقمٍ
 * عمرُه سنةٌ هو الرقمُ الذي لا يستطيع أحدٌ مراجعتَه** (D-219).
 * **فالعنوانُ صار يقول ما يملكه**، **والاختيارُ وحدَه هو من يحمل شرطَ
 * الحياة** (سبعةُ أيام — في الصفحة).
 *
 * ⚠️ **ودَينٌ موروثٌ يُكتب ولا يُخفى**: «N مشاركة» تعدّ نشراتِ Loopz
 * ضمنَها (بندٌ مفتوحٌ في `05_Todo` منذ D-273) — **وهذا السطحُ ورث العدَّ
 * كما هو، فيوم يُصلَح هناك يُصلَح هنا بلا لمسة** (D-261).
 *
 * **وبلا نداءٍ جديدٍ ولا هجرة**: الغرفُ مجلوبةٌ أصلاً لهذا التبويب
 * (D-194: ما يُدفع مرّةً لا يُدفع مرّتين).
 */
export function TalkedAboutWork({
  room,
  locale,
}: {
  /** **الغرفةُ المختارةُ في الصفحة** — غيابُها يُغيّب القسمَ كلَّه (D-181) */
  room: TalkRoom | null;
  locale: Locale;
}) {
  const t = getDict(locale);
  ...
  return (
    <BoardSection icon="comment" tone={SECTION_TONE.talked} title={t.peopleBoardTalked}>
      {/* **والبطاقةُ بطاقةُ `/talk` نفسُها بـ`hero`** — **لا نسخةٌ ثانية**
          (D-291/D-068): تدرّجُ الغطاء فيها صُحّح مرّتين، **ونسخةٌ ثانيةٌ
          تُصلَح مرّةً وتبقى الأخرى مقلوبة** (D-145). */}
      <TalkRoomCard room={room} locale={locale} hero />
    </BoardSection>
  );
}
