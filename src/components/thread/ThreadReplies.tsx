...
export function ThreadReplies({
   ...
   */
  /**
   * 🆕 **وردُّ صاحب بلس على رأيٍ يكون من أوائل الردود** (D-834، حكمُ
   * أحمد: «رد صاحب البلس على رفيو الأشخاص يكون من أوائل الردود **فقط
   * لا غير** — وفي مكانٍ آخر لن نميّزه، يكفيك جنب اسمه مكتوب بلس»).
   *
   * 🔑 **وحدُّها خيوطُ الآراء وحدَها**: `review` و`listReview` — **وهما
   * وجهان لشيءٍ واحدٍ عند القارئ** (رأيُ شخصٍ يُردّ عليه)، **وقاعدتان
   * لشيءٍ واحدٍ خلل** (D-145). **وغرفةُ النقاش والمنشورُ لا تُمسّان**:
   * **تلك ترتّب بالأصوات بحكمه** (D-305) — **وامتيازٌ يعلو صوتَ
   * الناس يفسد الترتيبَ الذي طلبه هو.**
   *
   * ⚠️ **والجذورُ وحدَها تُقسم** (نفسُ حجّة الأصوات أدناه): **مقارنٌ
   * يمسّ الأبناءَ يكسر الشجرة** — **وموضعُ الابن في المصفوفة لا يغيّر
   * أباه، لكنّ ترتيبَ إخوته يتبع المصفوفة.**
   * ⚠️ **والفرزُ مستقرّ**: المتساوون يبقون بترتيب الزمن — **فالقاعدةُ
   * تُقدّم ولا تُعيد الترتيب.**
   * ⚠️ **و`plan` في الصفّ يكفي**: العرضُ العامُّ يخفضه إلى `free` عند
   * انقضاء المدّة (الهجرة ١٥٦) — **فلا تاريخَ يُقرأ هنا ولا نداءَ
   * ثانٍ.**
   */
  if (!votes && (target.kind === "review" || target.kind === "listReview")) {
    const roots = all.filter((r) => !r.parentId);
    const kidsOf = all.filter((r) => r.parentId);
    const first = roots.filter((r) => isPlus(r));
    const rest = roots.filter((r) => !isPlus(r));
    if (first.length > 0 && rest.length > 0) {
      all.splice(0, all.length, ...first, ...rest, ...kidsOf);
    }
  }
