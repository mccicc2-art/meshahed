...
export function LibraryGrid({
  ...
  shows: GridItem[];
  movies: GridItem[];
  /** أنميك — **مسلسلاتٌ وأفلامٌ معاً**. 🆕 **ومنذ D-946 يُنزع المعلَّمُ
      أنمياً من تبويبه الأصليّ** (طلبُ أحمد: «الأنمي فقط في قائمة الأنمي») —
      نقضٌ لـD-182. والقاطعُ يعيش في `library/page.tsx` لا هنا: هذا المكوّن
      يعرض ما يصله. **وغيرُ المصنَّف (`null`) يبقى في «مسلسلاتي»** حتى
      تصنّفه الجولةُ الأولى. */
  anime?: GridItem[];
  ...
  /* 🔴 🆕 **مجموعةٌ واحدةٌ لكلِّ حالة** (D-789، بلاغُ أحمد بلقطتين:
     «الأنمي قائمة المكتمل مكرّرة في المكتبة»).

     **العطلُ كان في السطر أعلاه بعينه**: كان التجميعُ يقرأ الجوارَ —
     **يبدأ مجموعةً جديدةً كلّما تبدّلت حالةُ العنصر عن سابقه** — **وهو
     يفترض تجاوراً لا يملكه ولا يستطيع فرضَه.**

     🔑 **وأين انكسر الافتراض**: تبويبُ الأنمي **ليس قائمةً واحدة**، بل
     `[...shows, ...movies]` مصفّاةً (`library/page.tsx`) — **ومصفوفتان
     كلٌّ منهما مرتَّبةٌ بحالتها تُلحَق إحداهما بالأخرى فتعود الحالةُ
     مرّتين**: مكتملُ المسلسلات، ثمّ ما بعده، ثمّ مكتملُ الأفلام.
     **فرأى أحمدُ «المكتمل» رفّين.**

     ⚠️ **ثمّ ضاعف العطلَ مفتاحُ React**: الرفُّ يحمل `key={g.status}` —
     **ومجموعتان بالحالة نفسِها تحملان المفتاح نفسَه**، فيُطابق React
     الثانيةَ بالأولى **ويرسم فيها ملصقاتِ الأولى بعينها**. **ولذلك بدا
     الرفّان توأمين لا رفَّين مختلفين** (تسعةٌ وتسعة، وهما تسعةٌ وواحد)،
     **وفتحُ أحدهما كان يفتح الآخر** لأن `openGroups` مفهرسةٌ بالحالة.

     ⚖️ **والعلاجُ في التجميع لا في ترتيب الخادم**: **قائمةٌ تُبنى من
     مصدرين لا يُطلب منها أن تصل مرتَّبةً على هوى قارئها** — **ومن
     جمّع على افتراضٍ لا يملكه انكسر عند أوّل مصدرٍ ثانٍ.** والخريطةُ
     تحفظ ترتيبَ أوّلِ ظهور، **فترتيبُ الحالات كما أراده الخادمُ
     والمسلسلاتُ قبل الأفلام داخل كلِّ حالة** — **والذي تغيّر أنّ
     الحالةَ لا تتكرّر.** */
  const groups = useMemo(() => {
    if (!grouped) return [] as { status: LibraryStatus; items: typeof items }[];
    const byStatus = new Map<LibraryStatus, typeof items>();
    for (const x of items) {
      const st = (x.status ?? "watching") as LibraryStatus;
      const bucket = byStatus.get(st);
      if (bucket) bucket.push(x);
      else byStatus.set(st, [x]);
    }
    return [...byStatus].map(([status, group]) => ({ status, items: group }));
  }, [grouped, items]);
  ...
  return (
    <div>
      ...
      {/* 🆕 ⚖️ **وبابا «الإحصائيات» و«النشاط» يغيبان عن تبويب القوائم**
          (D-832، حكمُ أحمد بلقطةٍ محوَّطة: «هذي خلّيها كلّها صف واحد ·
          ممكن اكتيفتي والاحصائيات من اللستة»).
          📏 **والأربعةُ لا تجتمع في صفٍّ واحدٍ على الجوّال — مقيسٌ لا
          مفترَض**: «الإحصائيات» و«النشاط» ~١٢٠ لكلٍّ، و«قائمة جديدة»
          ١٠٢ و«قائمة ذكيّة» ٩٧، **والمجموعُ ~٤٦٢ في عرضٍ ٣٥٨** —
          **فالصفُّ الواحدُ يشترط أن يخرج اثنان.**
          🔑 **والخارجُ هما الأبعدُ عن السؤال**: **الإحصائياتُ والنشاطُ
          بابان عن مشاهدتك، وتبويبُ القوائم عن قوائمك** — **وهما
          حاضران في التبويبات الثلاثة الأخرى** فلا بابَ يُفقد.
          ⚠️ **والشرطُ هنا لا في الصفحة**: **التبويبُ حالةُ هذا المكوّن
          وحدَه** (D-521) — **وصفحةٌ تخمّن التبويبَ لتقرّر ما تمرّره
          تخطئ عند أوّل تنقّلٍ بلا رحلةِ خادم.** */}
      {underTabs && tab !== "lists" && (
        ...
        grouped ? (
          <div className="space-y-7">
            {groups.map((g) => {
              ...
              /* ⚖️ 🆕 **و«قيد المشاهدة» عادت ملصقاتٍ كبقيّة المجموعات**
                 (نقضُ D-443، طلبُ أحمد ٢٠ أغسطس بلقطةٍ للقسمين:
                 «طريقة عرض واتشينغ في المكتبة رجّعها مثل الشكل القديم
                 الي تحتها»).

                 **وحجّةُ D-443 كانت أنّ سؤالَ المجموعة يختلف** — «أين
                 وصلتُ» لا «ماذا عندي» — **وثمنُها الذي لم يُحسب أنّ
                 صفّين عريضين يأكلان الشاشةَ كلَّها** قبل أن يبدأ «لم
                 يبدأ»، **فتُقرأ المكتبةُ قائمتين لا مكتبةً**، وهو نقيضُ
                 ما بُنيت له الرفوف (D-422).
                 **والرقمان اللذان بُرِّر بهما الصفُّ لم يسقطا**:
                 `Cell` تمرّر `progress` و`count` إلى الملصق أصلاً —
                 **فالذي تغيّر الإطارُ لا المعلومة.** */
              /* **ومجموعةٌ بعنصرٍ واحدٍ لا تُسحب** (D-442/D-443، بلاغُ
                 أحمد: «إزالة الفراغ الكبير عند وجود عمل واحد»): **صفٌّ
                 بعنصرٍ واحد يترك أغلبَ عرضه فراغاً**، **والعنصرُ وحدَه
                 يأخذ العرضَ كلَّه.** */
                ...
                <PosterRail
                  ...
                  action={
                    <button
                      ...
                      className="text-12 text-muted hover:text-accent transition shrink-0 tabular-nums"
                    >
