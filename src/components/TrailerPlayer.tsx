...
export function TrailerPlayer({
  ...
  useEffect(() => {
    ...
    /* 🔴 🆕 **ومراقبُ القرب يتنحّى لمراقب الرؤية** (D-750): **هامشُه
       أوسعُ فيتقاطع أوّلاً دائماً** — **فلو ركّب فوراً لسبق قرارَ
       التشغيل وركّب صاحبَ الدور ساكناً.** **ومهلةُ صفرٍ تكفي**: نداءات
       المراقبَين تُسلَّم في دفعةٍ واحدة، **والمهلةُ تقع بعدهما معاً.**
       🔑 **والقاعدة: من سبق بالبنية لا بالحقّ فليُؤخَّر** — **وسباقُ
       مراقبين ليس خياراً، هو نتيجةُ هندستهما.** */
    let nearTimer = 0;
    ...
    const io = new IntersectionObserver(
      ([e]) => {
        ...
        if (e.isIntersecting && e.intersectionRatio >= 0.4) {
          ...
          setOwner(true);
          /* 🔴 🆕 **وصاحبُ الدور يبدأ بنفسه دائماً** (D-750، بلاغُ أحمد:
             «ما يبدأ الفيديو وهو صامت، ولازم أضغط الصوت، وبعد خمس ثوانٍ
             يمشي»).
             🔴 **الخمسُ ثوانٍ كانت مؤقّتَ نجاتي** (٣٫٥ ثمّ إعادةُ
             تحميلٍ وتخزين): **في D-745 جعلتُ `playVideo` مساراً لمن
             سبقه القرب** — **ومراقبُ القرب يسبق مراقبَ الرؤية دائماً**
             (هامشُه أوسع فيتقاطع أوّلاً)، **فصار المسارُ الذي لم أقسه هو
             الافتراضيَّ لأوّل بطاقةٍ على الشاشة، ونجاتي هي التي تُشغّل
             الفيديو.**
             🔑 **والقاعدةُ المستخلَصة**: **مسارٌ لم يُقس لا يُوضع حيث
             يقع الحملُ الغالب** — **ومن جعل المُثبَتَ احتياطاً
             والمجهولَ أصلاً فقد قلب الحجّة.** **وسباقُ مراقبين ليس
             خياراً، هو نتيجةُ هندستهما.**
             ⚠️ **والتحميلُ المسبقُ لم يُلغَ، تغيّرت ثمرتُه**: **يُعيد
             التحميلَ عند التوّلي — لكن من ذاكرةٍ دافئة** (جافاسكربت
             المشغّل وصورُه مخزَّنة). **قيمتُه تسخينُ الخبيئة لا إبقاءُ
             إطارٍ حيّ**، وهذا ما يشتريه فعلاً. */
          setBoot(1);
          mountedRef.current = true;
          setMounted(true);
  ...
  /* ⚖️ **ومسارُ النجاة حُذف** (D-750): **بُني في D-745 ليُنقذ
     `playVideo`** — **وقد سقط المسارُ الذي كان يحرسه، فحارسُه بلا عمل.**
     **وحارسٌ لطريقٍ أُغلق يُقرأ طريقاً قائماً.** */
