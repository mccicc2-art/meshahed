...
export function TabPager({
  ...
  useBeforePaint(() => {
    ...
    setTabDrag(0);
    /* **والتصفيرُ هنا صار لضغطة الشريط وحدَها** (D-523 → 🔄 D-530):
       السحبةُ تصفّر المستندَ معوَّضاً عند **بدء رحلتها** (انظر `finish`)
       فيصل هذا السطرَ صفراً أصلاً — **وضغطةُ التبويب بلا رحلةٍ فتصفيرُها
       في التزام قلبها كما كان.**
  ...
  useEffect(() => {
    ...
    /**
     * 🔄 🆕 **التعويضُ الرأسيُّ يعود — في موضعٍ جديدٍ وبنصفه الناجي**
     * (D-530، عودةُ آليّة D-304 التي نقضتها D-523).
     *
     * 📏 **تسجيلُ أحمد (٦٠ إطار/ث) حصر رمشةَ الهيدر في لحظة القلب**:
     * الهيدرُ لا يتغيّر محتواه هناك أصلاً، **لكنّ قفزةَ التمرير ٨٠٠→٠
     * في التزام القلب تنقل الشريطين اللاصقين من «ملتصقٍ» إلى «موضعهما
     * الطبيعيّ» — وiOS تهدم طبقتيهما لحظتَها** فيُرسمان خلفيّةً إطارين
     * (**والدوكُ `fixed` لم يرمش قطّ — وهو الشاهد**). و`will-change`
     * جُرّبت (`2a0fda6`) **فكسرت الإقلاعَ وارتدّت** (`fa04d5f`).
     *
     * **فالعلاجُ نقلُ القفزة لا ترميمُها**: تقع عند **بدء الرحلة** —
     * `scrollTo(0,0)` مع `dy = -y` على المسار في الإطار نفسِه،
     * **فلا تتحرّك بكسلةٌ واحدةٌ أمام العين** (الحسابُ أدناه في
     * `finish`)، **والشريطان لحظتَها في منتصف انزلاق العودة**
     * (`revealChrome` — D-524) **وانتقالُهما الجاري يبقيهما طبقتين
     * حيّتين عبر القفزة.** وعند القلب لا قفزةَ أصلاً — **فلا هدمَ ولا
     * رمشة.**
     *
     * ⚠️ **وما نُقض من D-523 هو حرفُها لا مقصدُها**: «المستندُ يبقى حيث
     * تركه القارئ طوال الرحلة» كان وسيلةً لثبات الصورة — **والثابتُ
     * الحقيقيُّ هو البكسل، وهو محفوظٌ هنا بالتعويض.** وعطلُ D-523
     * الأصليُّ (الدخولُ بعمق الصفحة السابقة) **كان من تسليح الجار عند
     * أسفل الشريط اللاصق، وعلاجُه (`top = scrollY`) باقٍ كما هو** —
     * فالجمعُ بين نصفَي D-304 وD-523 الناجيَين هو هذا القرار.
     * **والسحبةُ الملغاة لا تمرّ من هنا** — القفزةُ في فرع الالتزام
     * وحدَه، فمن تراجع بقي حيث كان (شرطُ D-523 محفوظ).
     */
    let dy = 0;
    ...
    const flushPending = () => {
      ...
      idxRef.current = p.i;
      /* والمستندُ عند الصفر منذ بدء الرحلة (D-530) — والسحبةُ الثانية
         تبدأ من فهرسٍ صحيحٍ وتعويضٍ نظيف */
      dy = 0;
    ...
    const finish = (s: NonNullable<typeof drag.current>, e: TouchEvent) => {
      ...
      /* 🆕 ⚡ **والكسوةُ تعود إلى حال الرأس مع بدء الرحلة لا بعدها**
         (D-524): رأسُ صفحةٍ بلا ترويسةٍ ولا شريطٍ حالٌ لا تُرسم،
         **وهنا تنزلق داخل زمن الحركة فلا تُرى.** **وانتقالُها الجاري
         شرطٌ للسطر التالي** (D-530): هو ما يُبقي طبقتيها حيّتين عبر
         القفزة. */
      revealChrome();
      /* 🔄 **القفزةُ المعوَّضة — هنا لا عند القلب** (D-530، الحجّةُ
         كاملةً أعلى الأثر): المستندُ إلى الصفر والمسارُ ينزاح بمثلها
         **في الإطار نفسِه قبل أيِّ رسم** — **فلا يتحرّك بكسلٌ أمام
         العين**، وعند القلب لا قفزةَ تهدم طبقتَي الشريطين. */
      const y0 = Math.max(0, Math.round(window.scrollY));
      dy = -y0;
      if (y0 > 0) {
        window.scrollTo(0, 0);
        setX(dx);
        /* ⚠️ **إجبارُ التخطيط هنا شرطُ صحّةٍ لا تحسين** (قِيس على
           المنشور قبل هذا السطر): كتابتا `transform` المتتاليتان —
           تثبيتُ التعويض بلا انتقالٍ ثمّ الرحلةُ بانتقالٍ — **تُدمجان
           في حسبةِ أنماطٍ واحدة، فيبدأ الانتقالُ من القيمة المرسومة
           القديمة ويحرّك `dy` من صفرٍ إلى قيمته على مدى الرحلة** —
           قفزةٌ كاملةٌ أمام العين. **والقراءةُ تفصل الكتابتين.** */
