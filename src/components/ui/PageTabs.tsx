...
export interface PageTab {
  ...
  href?: string;
  /**
   * 🆕 **`replace` — تبويبٌ لا يكدّس تاريخاً** (D-643، بلاغُ أحمد: «سهم
   * الرجوع يطلّعني من صفحته، ما يخلّيني أرجع بين التبويبات»).
   * **وهو نقضٌ لسلوكٍ لا لقاعدة**: التبويبُ **وجهٌ ثانٍ لصفحةٍ واحدة لا
   * وجهةٌ ثانية** — **ومن ضغط ثلاثةَ تبويباتٍ ثمّ «رجوع» يريد الخروجَ
   * لا التراجعَ خطوةً خطوة.** (وهي حجّةُ D-521 نفسُها المطبَّقةُ على
   * حقلٍ: «سجلٌّ لكلِّ ضغطةٍ يحوّل الرجوعَ إلى Backspace».)
   * ⚠️ **ولا يُعمَّم**: تبويبُ `/messages` وجهةٌ يُشارَك رابطُها، **وصفحةُ
   * الملفّ يُشارَك رابطُها الجذرُ لا تبويبُها.**
   */
  replace?: boolean;
  /**
  ...
  return (
    ...
    /* **`-mt-6` يبتلع حشو الصفحة فوقه (D-177، طلب أحمد بلقطتين):** الحاوية
       ...
       **والهامش السالب هنا لا حذفُ `py-6` من `main`:** ذاك يمسّ كلَّ صفحةٍ
       ...
       🔴 🆕 **والسحبةُ على مقاس ما فوق الشريط** (D-582، بلاغُ أحمد
       بلقطة: «فيه تداخل الكارد على التبويب»): `-mt-6` كُتبت لصفحاتٍ
       الشريطُ فيها أوّلُ المحتوى وفوقه حشوُ `main` (٢٤px) — **وملفُّ
       المستخدم (`autoHide=false`) إيقاعُه `space-y-5` وفوق شريطه بطاقةُ
       الأرقام**: **فكان الشريطُ المعتمُ (`z-20`) يصعد أربعةَ بكسلاتٍ فوق
       البطاقة (`z-10`) ويطمس حدَّها السفليّ** — مقيسةً على المنشور:
       `card.bottom = 353 · bar.top = 349`. **فالمقاسُ يتبع الوضعَ لا
       يُعمَّم**: `-mt-5` تصفّر فارقَ الإيقاع ولا تقلبه. */
    <Shell
      className={`${
        autoHide ? "chrome-sub -mt-6" : "relative -mt-5"
      } sticky top-[var(--sticky-top)] z-20 -mx-4 px-4 pt-1 bg-[color:var(--background)]${className ? ` ${className}` : ""}`}
    >
      ...
      {/* ⚖️ 🆕 **والحشوةُ الفارغةُ لا تُرسم في الشريط غير المتكيّف**
          (D-585، بلاغُ أحمد: «فيه هامش كبير فاضي احذفه»): `pb-2` بلا
          `extra` كانت ثمانيةَ بكسلاتٍ معتمةً تحت الخطّ في ملفّ
          المستخدم، **تُضاف إلى إيقاع الصفحة فيقرؤها القارئُ فراغاً بلا
          مالك.** وصفحاتُ `chrome-sub` تبقيها — خطُّها يحتاج نفَساً وهو
          يظهر ويختفي مع الكسوة. */}
      {extra ? (
        <div className="pt-3 pb-2">{extra}</div>
      ) : autoHide ? (
        <div className="pb-2" />
      ) : null}
    </Shell>
