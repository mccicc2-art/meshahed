...
export interface StatusThreadProps {
  ...
  progress?: number;
  /** بطاقةٌ حمراء — أحمرُ بمقدار التقدّم (D-784)، وكاملٌ لما لم يبدأ */
  dropped?: boolean;
  /**
...
export function StatusThread({
  ...
  /* **الأولوية: موقوفٌ ثم منتهٍ ثم جارٍ ثم محفوظ** — **وأخصُّ الحالات
     ...
     ⚖️ 🆕 **والأحمرُ بمقدار التقدّم لا كاملاً** (D-784، بلاغُ أحمد
     بلقطة: «الخط الأحمر أبغاه على حسب التقدم وليس كامل») — **ناقضاً
     تعليقَ `dropped` القديم «الشريط كله أحمر مهما كان التقدّم»**:
     **الخيطُ يقول شيئين بلونه وطوله — اللونُ الحكمَ والطولُ المسافةَ** —
     وأحمرُ كاملٌ فوق عملٍ أوقفتَه في حلقته الخامسة يكذب في نصفه الثاني.
     ⚠️ **وموقوفٌ لم يبدأ يبقى كاملاً** (كقاعدة «عندك» سطراً فوق):
     **صفرُ عرضٍ خيطٌ غائبٌ لا حالةٌ هادئة** — والأفلامُ كلُّها هنا،
     فلا تقدّمَ جزئيّاً لفيلم. */
  const full = watched || ((dropped || saved) && pct <= 0);
  const color = dropped
    ? "var(--error)"
    : watched || pct >= 100
      ? "var(--success)"
