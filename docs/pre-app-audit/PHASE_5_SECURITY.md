...
**`S-14` · P2** (رُفعت من P3): **ليست تأخّرَ إصدارةٍ وحدَه، بل مُدقِّقٌ مهجورٌ صراحةً في سلسلة أدوات البناء.** معيار القبول: ترقيةُ `eslint` إلى إصدارةٍ مدعومة (ومعها `eslint-config-next` المطابقُ لـ`next`)، **أو** قرارُ تثبيتٍ موثَّقٌ يذكر أن المُدقِّقَ لا يتلقّى إصلاحات.
