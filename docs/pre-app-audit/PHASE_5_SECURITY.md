...
| **T4** | **المتصفّح ← Supabase مباشرةً** | **RLS وحده** — بمفتاح `anon` المشحون لكل متصفّح | **72 جدولاً** · 143 دالّة `SECURITY DEFINER` معروضة | 🔴 **`SEC-ALERT-01`** |
...
| المقياس | القيمة |
|---|---:|
| جداول `public` (`relkind='r'`) | **72** |
| **عليها RLS** | **72 — مئة بالمئة** |
| بلا RLS | **0** |
| مجموع السياسات | **128** على **57** جدولاً |
| سياسات يظهر فيها `auth.uid()` في `USING` **أو** `WITH CHECK` | **119** |
| منها في `USING` | 85 |
| منها في `WITH CHECK` | 54 |
| **سياسات بلا `auth.uid()` إطلاقاً** | **9 — كلُّها `SELECT`** |
| سياسات تُسمّي `anon` | **3** |
| جداول عليها RLS **بلا سياسة** = **منعٌ افتراضيّ كامل** | **15** |

> 🔴 **وتصحيحُ رقمٍ نشرتُه أنا — والمراجعُ كان سيعتمد على الخطأ:** كتبتُ في §9.1 **«73 جدولاً»**، **والصحيح 72.** **عددتُ العرضَ المميّز `public_profiles` جدولاً وهو `relkind='v'` لا `'r'`** — و**73** هو عددُ العلاقات لا الجداول. **والرقمُ المعتمَد في التقرير كلِّه: 72 جدولاً · 128 سياسة.**
>
> ⚠️ **و«84» كانت ناقصةً لا خاطئة**: عدَّت `USING` وحدَها فأسقطت سياساتِ `INSERT` التي لا `USING` لها أصلاً. **والعدُّ الصحيح على الشرطين: 119 من 128.**

**والتسعُ الباقيةُ ليست ثغرةً — وهي كلُّها قراءةٌ لا كتابة:** ثلاثةُ جداولِ كتالوجٍ عامّة (`imdb_chart` · `imdb_ratings` · `title_meta` بـ`true`) · وأسماءٌ بديلةٌ موثَّقة (`title_aliases` بـ`verified`) · وقائمتان عامّتان بعَلَمهما (`user_lists` · `user_list_items` بـ`is_public`) · وصفُّ الشريك للإدارة (`am_admin()`) · **وسياستان تقرآن `true` لكلِّ مصادَق** (`communities` · `user_follows`) **وهما `S-12` في §9.1.** **فصفر سياسةِ كتابةٍ بلا `auth.uid()`.**
...
| WARN | 5 | `function_search_path_mutable` — **الأسماء محجوبة في النسخة العامّة** | ⚖️ **تنبيهٌ قديمٌ لم يعد يصف الحال**: **الجردُ المباشرُ في §9.6 يقول 170/170 دالّة `DEFINER` لها `search_path` مثبَّت — صفرٌ بلا تثبيت.** فإمّا أن التنبيهات التُقطت قبل تثبيتها أو أنها تعدّ دوالَّ خارجَ `public`. **والمصدرُ الأحدثُ هو الجردُ لا التنبيه**، **و`S-04` حُسم في §9.6 بالتأهيل لا بهذا السطر** |
...
| **IDOR — الجداول** | **72 جدولاً** | RLS على الجميع · **128 سياسة**، **119 منها تستعمل `auth.uid()`** | `SOURCE_VERIFIED` بنيوياً · **والتحقّق الفعليّ A/B `BLOCKED_BY_TEST_ENV`** |
...
| ~~`S-04`~~ | **يسقط كعيبٍ — ويبقى ثابتاً واجبَ الحفظ** | 🔴 **والتعليلُ صُحِّح**: **ليس منعَ `CREATE`** — **`TEMP` ممنوحٌ فعلاً لـ`anon` و`authenticated` فالتظليلُ ممكنٌ نظريّاً.** **والذي يُبطله تأهيلُ أسماء الجداول**: 549 مرجعَ علاقةٍ في أجسام الـ`DEFINER`، **صفرٌ منها غيرُ مؤهَّلٍ إلى جدولٍ في `public`** (§9.6) | **فحصٌ آليٌّ قبل الدمج** يرفض دالّةَ `DEFINER` فيها مرجعٌ غيرُ مؤهَّل — **فالثابتُ يُحفظ ولا يُترك للانتباه** |
...
- **RLS على 72 جدولاً من 72**، و**128 سياسة**، **119 منها تستعمل `auth.uid()`** (الأرقام صُحِّحت بعد جردٍ مباشر؛ كان مكتوباً 71 و126 و84، ثمّ 73 خطأً — **و`public_profiles` عرضٌ لا جدول**).
...
**`LOOPZ-AUD-0005`** (`watched_episodes_backup_133`): **غير مقروء عبر Data API إطلاقاً** — RLS مفعّلة بلا سياسة، **وهو أحدُ الخمسةَ عشرَ في §9.1**. **فالتوصيفُ الصحيح: دَينُ احتفاظٍ بنسخةٍ زائدة، لا انكشافٌ مباشر** — **والدرجةُ تنزل عن `P1-CANDIDATE` تبعاً لذلك.**

⚠️ **ولا يسقط البند**: نسخةٌ بـ3510 صفوفٍ من بيانات مشاهدةِ أعضاء **تبقى واجبةَ الحذف** (أقلُّ بياناتٍ محفوظة)، **ويبقى الوصولُ إليها ممكناً عبر دالّةِ `DEFINER` إن وُجدت** — فهو داخلَ نطاق `LOOPZ-AUD-0040` لا خارجه. **والتخفيفُ في التوصيف لا في الوجوب.**
...
**البنية:** **72 جدولاً في `public` · RLS مفعَّل على 72/72 · 128 سياسة** (و`public_profiles` **عرضٌ** لا جدول — وهو `V-01` في §9.6).
...
|---|---|
| `LOOPZ-AUD-0040` · **P1** | **التحقّقُ الحيُّ من سبعِ دوالِّ القراءةِ بمعرّف و`FW-05` و`V-01` يحتاج حسابَين في بيئةٍ معزولة** — `BLOCKED_BY_TEST_ENV` |
