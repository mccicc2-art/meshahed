...
| `S-03` | **الدرجة مؤجَّلة** | عرضٌ واحد بامتياز المُعرِّف (تنبيه ERROR الوحيد) — **الاسم محجوب** | **الأعمدة والجداول المصدرية وgrants وهل البيانات عامّة بالقصد** — قبل تثبيت الدرجة |
| `S-04` | **الدرجة مؤجَّلة** | خمس دوالّ بامتياز المُعرِّف بلا `SET search_path` — **والأسماء محجوبة** | إثبات صلاحية `CREATE` لدورٍ عامّ على مخطَّطٍ في المسار وعدم تأهيل الأسماء داخلها **قبل** تثبيت الدرجة |
...
| الدوالّ القارئة بلا حارس | 🔴 **لم تُصنَّف بعد** (§8.5) |
| وراثة `PUBLIC` و`authenticated` | 🔴 **لم تُراجَع** — و`S-05` يثبت أنها تُبطل السحب الموجَّه |
