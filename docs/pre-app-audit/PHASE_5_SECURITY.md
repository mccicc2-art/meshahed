...
| **T4** | **المتصفّح ← Supabase مباشرةً** | **RLS وحده** — بمفتاح `anon` المشحون لكل متصفّح | **73 جدولاً** · 143 دالّة `SECURITY DEFINER` معروضة | 🔴 **`SEC-ALERT-01`** |
...
| **IDOR — الجداول** | **73 جدولاً** | RLS على الجميع · **128 سياسة**، 84 منها تستعمل `auth.uid()` | `SOURCE_VERIFIED` بنيوياً · **والتحقّق الفعليّ A/B `BLOCKED_BY_TEST_ENV`** |
...
- **RLS على 73 جدولاً من 73**، و**128 سياسة** (الرقمان صُحِّحا بعد الجرد الصفّيّ في §9.1؛ كان مكتوباً 71 و126)، و84 منها تستعمل `auth.uid()`.
